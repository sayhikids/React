...
public class JwtAuthenicationFilter extends OncePerRequestFilter {
	...
	private String parseBearerToken(HttpServletRequest request) {
		...
		//bearerToken : 토큰을 전공하면 브라우저에 토큰의 키 이름을 나타냅니다.
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			...
			if(token != null && !token.equalsIgnoreCase("null")) {
				...
				//principal 아래 생성자에서 첫번째 파라미터인 userId 는 해당 ID 로 부여된 토큰을 filter 가 검증할 수 있는 key 입니다.
				//이 값은 주어진 변수 이름 그대로 컨트롤러에서 사용할 수 있는데, 그 제공자로 @Auth...Principal 이라는 어노테이션으로 제공합니다.
